...
Similarly, the user must enter the entries for each row in the initial tableau. Provided any *m<sub>x</sub>n* matrix (m rows and n columns), the user must enter exactly *n* values for *m* distinct rows. Entries in each row are separated by spaces, and rows are separated by semicolons.
